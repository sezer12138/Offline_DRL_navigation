...
class turtlebot_env(gym.Env):
    ...
    def _set_init_pose(self):
        ...
        if self.choice:    
            ...
            self.desired_point.x = random.uniform(4.5, 6)
            self.desired_point.y = random.uniform(-4, -3)
        
        self.choice = not self.choice

        # self.desired_point.x = random.uniform(2.5, 7)
        # self.desired_point.y = random.uniform(-4, -2.5)
    ...
    def _is_done(self, odometry, laser_scan):
        ...
        if self.is_in_desired_position(odometry.pose.pose.position):
    ...
    def _compute_reward(self, observations, odometry, action, done):
        ...
        reward = distance_reward*300 + reward_collision + velocity_reward + arrive_reward
        ...
        return reward, arrive_reward, 300*distance_reward, reward_collision, velocity_reward
    ...
    def step(self, action):
        ...
        reward, arrive_reward, distance_reward, reward_collision, velocity_reward = self._compute_reward(new_observation, odometry, action, done)
