...
if __name__ == '__main__':
    env_name = "function_ddpg"
    rospy.init_node('function_ddpg', anonymous=True, log_level=rospy.WARN)
    env=turtlebot_env()
    rospy.loginfo("Gym environment done")
    # obtain the path of the current folder
    current_folder_path = os.getcwd()
    outdir = current_folder_path + '/training_results/function_ddpg_results/'
    torch.manual_seed(0)
    ...
    for i_episode in range(num_episodes):
        ...
        for t in range(100):  # replace 100 by the length of an episode
            ...
            goal_reward = other_reward["goal_reward"]
            ...
            episode_goal_reward += goal_reward
        ...
        writer.add_scalar("reward/arrive_reward", episode_arrive_reward, i_episode)
        writer.add_scalar("reward/goal_reward", episode_goal_reward, i_episode)
        writer.add_scalar("reward/reward_collision", episode_collision_reward, i_episode)
        # writer.add_scalar("velocity_reward", episode_velocity_reward, i_episode)

        if i_episode % 10 == 0 and len(replay_buffer) > agent.batch_size:
