...
class turtlebot_env(gym.Env):
    def __init__(self):
        ...
        # self.desired_point.x = random.uniform(-6, 6)
        # self.desired_point.y = random.uniform(3, 6)
    ...
    def _set_init_pose(self):
        ...
        """
        initial_x = random.uniform(4.5, 6)
        initial_y = random.uniform(-4, -3)
        # initial_x = random.uniform(-1, 1)
        # initial_y = random.uniform(0, 2)
        initial_z = 0.0
        self.desired_point.x = random.uniform(2.5, 3.5)
        self.desired_point.y = random.uniform(-4, -3)
        # self.desired_point.x = random.uniform(-6, 6)
        # self.desired_point.y = random.uniform(3, 6)
    ...
    def _init_env_variables(self):
        ...
        self.previous_arrive_reward = None
        self.previous_reward_collision = None
        return True
    ...
    def _compute_reward(self, observations, odometry, action, done):
        ...
        # distance_reward = 100 * (self.previous_distance_from_des_point - distance_from_des_point)
        ...
        reward_collision = 0
        min_laser_range = sum(heapq.nsmallest(3, observations[:-4])) / 3
        laser_data = observations[:-4]
        # if not self.previous_reward_collision:
        #     self.previous_reward_collision = np.exp(min_laser_range) - 1
        reward_collision = np.exp(min_laser_range) - 1
        # reward_collision += sum(laser_data) - 36
        ...
        # velocity_reward = linear_punish_reward + angular_punish_reward

        # if not self.previous_arrive_reward:
        #     self.previous_arrive_reward = np.exp(-(distance_from_des_point-5))
        goal_reward = np.exp(-(distance_from_des_point-5))
        arrive_reward = 0
        if done:
            if self.is_in_desired_position(current_position):
                arrive_reward += 100
        ...
        reward = reward_collision + arrive_reward + goal_reward
        
        # print("reward={}".format(reward))
        self.previous_distance_from_des_point = distance_from_des_point
        self.previous_arrive_reward = np.exp(-(distance_from_des_point-5))
        self.previous_reward_collision = np.exp(min_laser_range)
        ...
        return reward, arrive_reward, goal_reward, reward_collision
    ...
    def step(self, action):
        ...
        reward, arrive_reward, goal_reward, reward_collision = self._compute_reward(new_observation, odometry, action, done)
    ...
    def _set_robot_state(self, rx, ry, rz, gx, gy):
        ...
        goal_state.model_name = 'round_tin_top'
